...
(() => {
	...
	function resolveDeviceImageSrc(imagePath) {
		if (!imagePath) {
			return "";
		}
	...
	function createDeviceCardHTML(device, index, viewDetailsText) {
		const deviceLink = normalizeDeviceLink(device.link);
		const linkAttrs = deviceLink
			? ' href="' +
				escapeHtml(deviceLink) +
				'" target="_blank" rel="noopener noreferrer"'
			: "";
		const cursorClass = deviceLink ? "cursor-pointer" : "cursor-default";
		const imgSection =
			...
			escapeHtml(resolveDeviceImageSrc(device.image)) +
			'" alt="' +
	...
	function initDevicesPage() {
		...
		brandTabs.forEach((tab) => {
			const clickHandler = () => {
				...
				const brandDevices =
					brand === "all"
						? Object.values(devicesData).flat()
						: devicesData[brand] || [];
