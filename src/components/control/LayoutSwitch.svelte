<script lang="ts">
	...
	onMount(() => {
		...
		const allowSwitch = siteConfig.postListLayout.allowSwitch;

		if (!allowSwitch) {
			userPreference = defaultLayout;
			updateStorage(defaultLayout);
		} else if (sessionLayout === "list" || sessionLayout === "grid") {
			...
			setTimeout(() => {
				...
				if (!siteConfig.postListLayout.allowSwitch) {
					userPreference = siteConfig.postListLayout
						.defaultMode as LayoutMode;
					updateStorage(userPreference);
				} else if (saved === "list" || saved === "grid") {
