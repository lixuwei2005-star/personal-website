...
async function resolveImageToDataUri(imagePath: string): Promise<string> {
	if (!imagePath) {
		return "";
	}
	...
	return `data:${getMimeType(resolvedPath)};base64,${imageBuffer.toString("base64")}`;
}
...
export async function GET({
	...
	// Avatar + icon: resolve local/public/remote sources to a data URI.
	const avatarBase64 = await resolveImageToDataUri(profileConfig.avatar || "");
