...
class MusicPlayerStore {
	...
	private createInitialState(): MusicPlayerState {
		return {
			...
			currentTime: 0,
			duration: 0,
			volume: clampVolume(musicPlayerConfig.defaultVolume),
			isMuted: false,
			isShuffled: musicPlayerConfig.defaultShuffle === true,
			isRepeating: 0,
			showPlaylist: false,
			errorMessage: "",
			showError: false,
			isExpanded: musicPlayerConfig.defaultExpanded === true,
			isHidden: false,
	...
	private loadLocalPlaylist(): void {
		const configuredPlaylist =
			Array.isArray(musicPlayerConfig.localPlaylist) &&
			musicPlayerConfig.localPlaylist.length > 0
				? musicPlayerConfig.localPlaylist
				: LOCAL_PLAYLIST;
		this.state.playlist = configuredPlaylist.map((track) => ({ ...track }));
		if (this.state.playlist.length === 0) {
	...
	togglePlaylist(): void {
		if (musicPlayerConfig.showPlaylistPanel === false) {
