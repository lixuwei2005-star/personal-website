...
const WALLPAPER_MODE_DEFAULT_STORAGE_KEY = "wallpaperModeDefault";
...
function isValidWallpaperMode(mode: string | null): mode is WALLPAPER_MODE {
	return !!mode && VALID_WALLPAPER_MODES.includes(mode as WALLPAPER_MODE);
}
...
function getSystemTheme(): LIGHT_DARK_MODE {
	if (
...
export function getStoredWallpaperMode(): WALLPAPER_MODE {
	...
	const storedDefaultMode = localStorage.getItem(
		WALLPAPER_MODE_DEFAULT_STORAGE_KEY,
	);

	if (siteConfig.wallpaperMode.showModeSwitchOnMobile === "off") {
		localStorage.setItem("wallpaperMode", defaultMode);
		localStorage.setItem(WALLPAPER_MODE_DEFAULT_STORAGE_KEY, defaultMode);
		return defaultMode;
	}
